...
export const About: FC<AboutProps> = memo(
  ({ quote, bio, details, stats, statsLabels, portraitAlt, userPhoto, isRtl }) => {
          ...
          <div className="grid items-center grid-cols-1 gap-24 lg:grid-cols-2">
            <div ref={photoRef} className="reveal-scale relative group">
              <div className="aspect-[4/5] overflow-hidden rounded-sm accent-border relative z-10 shadow-2xl bg-[#1A1A1A]">
                {/* Intentional duplicate of Hero photo: this copy is the primary portrait content. */}
                <img
                  src={userPhoto}
                  alt={portraitAlt}
            ...
            <div className="space-y-16" dir={isRtl ? 'rtl' : 'ltr'}>
              ...
              <div ref={statsRef} className="reveal grid grid-cols-3 gap-8 pt-4">
                <StatItem value={stats.exp} label={statsLabels.exp} />
                <StatItem value={stats.projects} label={statsLabels.projects} />
                <StatItem value={stats.eng} label={statsLabels.eng} />
              </div>

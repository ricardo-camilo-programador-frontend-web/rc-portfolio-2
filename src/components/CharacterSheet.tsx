...
import { Award, Heart, Package, Shield, Star, Zap } from 'lucide-react'
...
export const CharacterSheet: FC<Props> = memo(({ character }) => {
  const xpProgress = character.xp % 100

  return (
    <div
      className="glass rounded-xl p-6 rpg-shadow border-l-4 border-red-600"
    ...
    >
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h2 className="text-xl font-cinzel font-bold text-white tracking-widest">
            {character.name}
          </h2>
          <p className="text-red-500/80 text-xs font-mono uppercase tracking-tighter">
      ...
      <div className="space-y-4">
        <div className="space-y-1">
          <div className="flex justify-between text-xs font-mono">
            <span className="flex items-center gap-1">
              <Heart size={12} className="text-red-500" /> VITALIDADE
            </span>
            <span>
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-xs font-mono">
            <span className="flex items-center gap-1">
              <Star size={12} className="text-yellow-500" /> EXPERIÊNCIA
            </span>
        ...
        <div className="pt-4">
          <h3 className="text-xs font-mono mb-2 flex items-center gap-1 uppercase opacity-50">
          ...
          <div className="flex flex-wrap gap-2">
            {character.inventory.map(item => (
              <span
                key={item}
                className="text-[10px] bg-white/5 border border-white/10 px-2 py-1 rounded text-zinc-300"
              >
                {item}
              </span>
            ))}
...
const StatItem: FC<{ label: string; value: number; icon: React.ReactNode }> = memo(
  ({ label, value, icon }) => (
    <div

...
export const CharacterSheet: FC<Props> = memo(({ character }) => {
  ...
  return (
    <div className="glass rounded-xl p-6 rpg-shadow border-l-4 border-red-600" style={{ contain: 'layout style paint' }}>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-red-500/80 text-xs font-mono uppercase tracking-tighter">{character.archetype} • Nível {character.level}</p>
        </div>
      </div>
...
const StatItem: FC<{ label: string; value: number; icon: React.ReactNode }> = memo(({ label, value, icon }) => (
  <div className="flex items-center justify-between p-2 bg-white/5 rounded border border-white/5" style={{ contain: 'layout style' }}>
